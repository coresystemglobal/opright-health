...
- [x] 🟢 Paystack subscription billing (Payments PR 2) — real recurring collection on Paystack (replaces the placeholder Stripe path): `createSubscription` returns a Paystack checkout URL and activates via webhook; `cancel` (period-end or immediate), `downgrade` (scheduled to next renewal via `pending_plan_type`), `upgrade` (immediate re-subscribe), `reactivate`; signature-verified subscription webhook (`charge.success`→renew, `invoice.payment_failed`→past-due+grace, `subscription.disable`→cancelled) replacing the no-op stub; new `paystack_customer_id`/`billing_email`/`subscription_status` on Tenant (migration 070); Redis sub-context cache invalidated on every change; endpoints `POST /api/billing/{cancel,downgrade,reactivate}` + `GET /api/billing/history`
- [x] 🟢 Subscription lifecycle cron + usage metering (Payments PR 3) — `subscription-lifecycle.service` sweeper (daily, wired in `core/index.ts` like dunning): expired trials → `past_due` + 3-day grace, grace lapsed → `cancelled` + tenant **suspended**, cancel-at-period-end finalized once the period ends; each transition invalidates the sub-context cache and denormalizes `tenant.subscription_status`. Wired the `lab_tests_count` usage counter on test-order create. *(Still open: appointments_count/storage_used_mb writers — need tenant/file-size plumbing; mounting `checkPlanLimits`.)*
...
- [x] 🟢 Labs (and pharmacies) as standalone tenants — **PR A**: `Tenant.facility_type` (hospital/laboratory/pharmacy/clinic/diagnostic_center/platform/other, migration 063) + `Laboratory`/`Pharmacy` facility-profile tables linked to `Tenant` (migrations 064/065), mirroring `Hospital`. **PR B**: lab data tables tenant-scoped — nullable `tenant_id` on `lab_tests`/`test_orders`/`test_results` (migration 066), `LaboratoryService` threads `tenant_id` through every method (creates stamp it, reads filter, by-id mutations verify ownership), and the lab-test catalog is now per-tenant (`test_code` unique per `(tenant_id, test_code)`). Legacy tenant-less rows are invisible to scoped queries (backfill on deploy if needed)
- [x] 🟢 Laboratory / Pharmacy facility-profile CRUD — tenant-scoped `/api/laboratories` and `/api/pharmacies` (create/list/get/update/soft-delete); `facility:view`/`facility:manage` RBAC (granted to admin); each tenant manages only its own profile rows
- [x] 🟢 Hospital table reconnected to `Tenant` — nullable `tenant_id` on `hospitals` (migration 067) + `tenantMiddleware` and tenant-scoping across the hospital service/routes (a tenant only sees/edits its own hospital); fixed the invoice-PDF header to use the invoice's own tenant's hospital (was `Hospital.findOne()` = first global row)
